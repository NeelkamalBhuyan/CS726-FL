...
def main():
    ...
    server.initialize_dicts(online(clients))
    ...
    all_metrics = print_stats(0, server, clients, client_num_samples, args, args.use_val_set)
    ...
    for i in range(num_rounds):
        ...
        if (i + 1) % eval_every == 0 or (i + 1) == num_rounds:
            all_metrics = print_stats(i + 1, server, clients, client_num_samples, args, args.use_val_set)
    ...
    metrics_df = pd.DataFrame(df_main, columns = ['round', 'model train acc', 'model train loss', 'model test acc', 'model test loss'])
...
def print_stats(
    num_round, server, clients, num_samples, args, use_val_set):
    all_metrics = []

    train_stat_metrics = server.test_model(clients, set_to_use='train')
    metrics_return = print_metrics(train_stat_metrics, num_samples, prefix='train_')
    #writer(num_round, train_stat_metrics, 'train')
    all_metrics = all_metrics + metrics_return

    eval_set = 'test' if not use_val_set else 'val'
    test_stat_metrics = server.test_model(clients, set_to_use=eval_set)
    metrics_return = print_metrics(test_stat_metrics, num_samples, prefix='{}_'.format(eval_set))
